...
def game_team_tile(meta,score,possession,live,score_color=WHITE):
    ...
    code_row=render.Box(width=info_width,height=7,child=render.Column(children=[spacer_h(1),render.Box(width=info_width,height=6,child=render.Row(children=[render.Text(meta["code"],font="tom-thumb",color=WHITE)],main_align="center",cross_align="center"))],main_align="start",cross_align="stretch"))
...
def parse_competition(event,timezone):
    ...
    for c in competitors:
        ...
        team=c.get("team"); m=team_meta(team); cid=s(c.get("id"),"")
    ...
    status=event.get("status"); stype=status.get("type") if type(status)=="dict" else {}; state_raw=s(stype.get("state")) if type(stype)=="dict" else "pre"; state="live" if state_raw=="in" else ("final" if state_raw=="post" else "pre")
...
def parse_scoreboard(data,config):
    ...
    tz=s(config.get("timezone"),"America/New_York"); games=[]
    for event in events:
...
def weekly_rollover_start():
    now=time.now().in_location("America/New_York"); weekday=now.format("Mon"); days_back={"Tue":0,"Wed":1,"Thu":2,"Fri":3,"Sat":4,"Sun":5,"Mon":6}.get(weekday,0)
...
def calculated_record(games,team,season_type):
    ...
    for g in games:
        if g["state"]!="final" or g.get("season_type")!=season_type: continue
        if g["away"]["code"]==team: us=g["away_score"]; them=g["home_score"]
        elif g["home"]["code"]==team: us=g["home_score"]; them=g["away_score"]
        else: continue
        if us>them: wins+=1
...
def main(config):
    ...
    cycle_seconds=i(config.get("all_games_cycle_time"),8)
    return render.Root(delay=cycle_seconds*1000,show_full_animation=True,child=render.Animation(children=frames))
def get_schema():
    return schema.Schema(version="1",fields=[schema.Dropdown(id="mode",name="Game Mode",desc="Track one team or cycle all NFL games.",icon="gear",default="team",options=[schema.Option(display="Specific Team",value="team"),schema.Option(display="All Games",value="all")]),schema.Dropdown(id="team",name="Team Focus",desc="Team used in Specific Team mode.",icon="gear",default="PHI",options=teamOptions),schema.Dropdown(id="all_games_cycle_time",name="All Games Cycle Time",desc="Seconds each game is shown while cycling All Games.",icon="timer",default=8,options=[schema.Option(display="3 seconds",value=3),schema.Option(display="4 seconds",value=4),schema.Option(display="5 seconds",value=5),schema.Option(display="6 seconds",value=6),schema.Option(display="8 seconds",value=8),schema.Option(display="10 seconds",value=10),schema.Option(display="12 seconds",value=12),schema.Option(display="15 seconds",value=15)]),schema.Color(id="pregame_date_color",name="Pregame Date Color",desc="Pregame weekday/date color.",icon="brush",default=WHITE),schema.Color(id="pregame_time_color",name="Pregame Time Color",desc="Pregame kickoff-time color.",icon="brush",default=WHITE),schema.Color(id="quarter_color",name="Quarter Color",desc="Live-game quarter color.",icon="brush",default=WHITE),schema.Color(id="clock_color",name="Clock Color",desc="Live-game clock color.",icon="brush",default=WHITE),schema.Color(id="field_color",name="Field Position Color",desc="Live-game field position color.",icon="brush",default=WHITE),schema.Color(id="final_color",name="Final Color",desc="Final-state text color.",icon="brush",default=WHITE)])
